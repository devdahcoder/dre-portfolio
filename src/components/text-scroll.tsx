...
const TextScroll = (props: Props) => {
	...
	return (
		<divclass={`flex flex-row items-center overflow-hidden py-10`}>
			<motion.div
				class={`flex flex-row items-center gap-x-12`}
				style={{ x }}
			>
				{words?.map((word: string, index: number) => {
					return (
						<div
							key={index}
							class={`w-max flex flex-row items-center gap-x-5 text-4xl text-white`}
						>
							{word}{" "}
							<span>
								<svg
